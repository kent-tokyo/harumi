...
**기존 PDF의 레이아웃을 유지하며 CJK/다국어 텍스트를 추출·교체·재배치하는 순수 Rust PDF 엔진.**

harumi는 기존 PDF에서 위치 정보를 포함한 텍스트를 추출하고,
번역하거나 교체한 결과를 원본 페이지 레이아웃을 깨뜨리지 않고 다시 써넣습니다.
...
**주요 용도：**
- PDF 번역 파이프라인 (추출 → 번역 → 레이아웃 유지 상태로 다시 쓰기)
...
| 충돌의 구조적 관계 분류가 필요한 경우 (같은 행, 인접 행, 헤더/푸터 등) | `classify_collisions(regions, collisions) -> Vec<ClassifiedCollision>` — 각 `Collision`에 `CollisionKind`（`SameRegion`·`SameRow`·`AdjacentRow`·`SameColumn`·`HeaderFooter`·`Unknown`）、각 영역의 `LayoutRegionRole` 및 `CollisionSeverity`（`Minor`·`Moderate`·`Major`）를 부여；`plan_text_for_regions*`의 `RegionFitPlan.collisions`가 직접 `Vec<ClassifiedCollision>`을 반환（v1.12.0+, severity v1.14.0+） |
| PlacedBox 크기만으로 충돌 심각도를 평가하고 싶은 경우 (LayoutRegion 없이) | `collision_severity(overlap_area, box_a_area, box_b_area) -> CollisionSeverity` — LayoutRegion 없이 PlacedBox만 사용하는 호출자를 위한 독립 심각도 계산 함수；box_area가 0이면 절대 pt² 임계값으로 대체（v1.14.0+） |
| 폼/테이블 PDF에서 레이블/값 영역 쌍을 추출하고 싶은 경우 | `extract_label_value_pairs(regions) -> Vec<LabelValuePair>` — 각 `LayoutRegionRole::LeftLabel` 영역을 같은 행의 `RightValue` 형제와 쌍으로 묶음；밀집된 SDS/양식 PDF 감지 및 번역 컨텍스트 구축에 유용（v1.14.0+） |
